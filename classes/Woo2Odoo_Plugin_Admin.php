...
final class Woo2Odoo_Plugin_Admin {
	...
	public function settings_screen() {
		...
		<div class="wrap woo2odoo-plugin-wrap">
			...
			<?php } else { ?>
			<h2><?php esc_html_e( 'Tools', 'woo2odoo-plugin' ); ?></h2>
			<table class="form-table" role="presentation">
				<tbody>
					<tr>
						<th scope="row"><?php esc_html_e( 'Delete cache?', 'woo2odoo-plugin' ); ?></th>
						<td><p><?php esc_html_e( 'This is useful if you are experiencing issues with the plugin.', 'woo2odoo-plugin' ); ?></p>
							<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
								...
								<?php wp_nonce_field( 'woo2odoo_plugin_delete_cache', 'woo2odoo_plugin_delete_cache' ); ?>
								<?php submit_button( __( 'Delete cache', 'woo2odoo-plugin' ), 'secondary' ); ?>
							</form>
						</td>
					</tr>
					<tr>
						<th scope="row"><?php esc_html_e( 'Sync order:', 'woo2odoo-plugin' ); ?></th>
						<td>
							<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
								<input type="hidden" name="action" value="woo2odoo_sync_order">
								<input type="hidden" name="woo2odoo_sync_order" value="1">
								<?php wp_nonce_field( 'woo2odoo_plugin_sync_order', 'woo2odoo_plugin_sync_order' ); ?>
