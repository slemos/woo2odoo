...
- **Comando `wp woo2odoo backfill`**: vincula pedidos antiguos con su Sale Order + Boleta ya existentes en Odoo (búsqueda por `origin`, la referencia autoritativa) y los marca `synced`, **sin crear nada y sin generar pago** — para pedidos conciliados manualmente. Prefiere la boleta apuntada por la meta legacy `_odoo_invoice_id` si pertenece al SO. Flags `--before`, `--wc-status`, `--limit`, `--dry-run`, y `--missing-invoice` (procesa los que ya tienen Sale Order pero les falta la boleta — p.ej. pedidos que una versión previa sincronizó a medias, incluso corrigiendo un `_odoo_sale_order_id` desfasado, ya que se re-vincula por `origin`). Método `Woo2Odoo_Order_Manager::backfill_from_odoo()`.
