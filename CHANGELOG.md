...
- **Comando `wp woo2odoo backfill`**: vincula pedidos antiguos con su Sale Order + Boleta ya existentes en Odoo (búsqueda por `origin`) y los marca `synced`, **sin crear nada y sin generar pago** — para pedidos conciliados manualmente. Prefiere la boleta apuntada por la meta legacy `_odoo_invoice_id` si pertenece al SO. Idempotente (solo toca pedidos sin `_odoo_sale_order_id`); flags `--before`, `--wc-status`, `--limit`, `--dry-run`. Método `Woo2Odoo_Order_Manager::backfill_from_odoo()`.
